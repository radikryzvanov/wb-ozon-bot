...
export async function buildInfographic(photoBuffer, title, bullets) {
  ...
  const bulletLines = bullets
    .map((b, i) => `<text x="60" y="${880 + i * 60}" font-size="34" font-weight="bold" fill="white" font-family="Roboto, DejaVu Sans, sans-serif">✓ ${escapeXml(b)}</text>`)
    .join("\n");
  ...
  const svgOverlay = `
    <svg width="${WIDTH}" height="${HEIGHT}">
      ...
      <text x="60" y="95" font-size="52" font-weight="900" fill="white" font-family="Roboto, DejaVu Sans, sans-serif">${escapeXml(title)}</text>
